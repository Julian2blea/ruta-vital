...
class GlucoseReading(models.Model):
    ...
    def _classify(self):
        ...
        especific_context = self.context

        # La hipoglucemia es universal; el contexto no cambia este umbral
        if value < 70:
            return 'hypoglycemia'

        if especific_context == 'fasting':
            # Umbrales en ayunas
            if value <= 99:
                return 'normal'
            elif value <= 125:
        ...
        elif especific_context == 'postprandial':
            # Umbrales postprandiales (2 horas después de comer)
            # Un valor de 120 mg/dL se considera normal después de comer, pero algunos expertos usan 140 mg/dL como límite superior para normalidad.
            if value <= 139:
                return 'normal'
            elif value <= 199:
                return 'prediabetes'
    ...
    def __str__(self):
        return f"{self.user.login} — {self.glucose_value} mg/dL [{self.context}] ({self.status})"
...
class GlucoseRecommendation(models.Model):
    ...
    immediate_action  = models.CharField(max_length=500)
    additional_advice = models.CharField(max_length=500, blank=True)
    when_to_measure   = models.CharField(max_length=150)
